...
const path string = "/.well-known/acct-image"
...
func serveHTTP(responsewriter http.ResponseWriter, request *http.Request) {
	...
	var resource string
	{
		...
		if !found {
			const code int = http.StatusBadRequest
			http.Error(responsewriter, http.StatusText(code), code)
			log.Error("resource not provided in acct-image request")
			return
		}
		if 1 != len(resources) {
			const code int = http.StatusBadRequest
			http.Error(responsewriter, http.StatusText(code), code)
			log.Error("too many resources provided in acct-image request")
			return
		}
...
func serveGET(responsewriter http.ResponseWriter, request *http.Request, resource string) {
	...
	var url string = activity.Image.URL
	if "" == url {
